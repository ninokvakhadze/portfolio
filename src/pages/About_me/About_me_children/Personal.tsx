...
export default function Personal() {
  const {   setDesktopTitleButtons, toggleHandler  } = useAboutMe();


  return (
    <PersonalDiv>
      <FileDiv onClick={() => {toggleHandler("first"); setDesktopTitleButtons((prev) => ({ ...prev, button1: true }))}}>
        <FileArrow src={fileArrow} />
        <FileImg src={fileImg1} />
        <FileName>personal</FileName>
      </FileDiv>
      <FileDiv onClick={() => {console.log("Clicked"); toggleHandler("second"); setDesktopTitleButtons((prev) => ({ ...prev, button2: true }))}}>
        <FileArrow src={fileArrow} />
